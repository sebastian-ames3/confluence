...
class TestRetranscribeEndpoint:
    """Test the retranscribe endpoint for existing 42macro videos."""

    def test_retranscribe_endpoint_exists(self):
        """Verify retranscribe endpoint is defined."""
        collect_path = Path(__file__).parent.parent / "backend" / "routes" / "collect.py"
        content = collect_path.read_text()

        assert '@router.post("/retranscribe/42macro")' in content, \
            "Retranscribe endpoint should exist"
        assert "async def retranscribe_42macro_videos" in content, \
            "retranscribe_42macro_videos function should exist"

    def test_retranscribe_checks_for_existing_transcripts(self):
        """Verify retranscribe checks if videos already have transcripts."""
        collect_path = Path(__file__).parent.parent / "backend" / "routes" / "collect.py"
        content = collect_path.read_text()

        assert "has_transcript = len(content_text) > 200" in content, \
            "Should check content_text length to determine if transcript exists"
        assert 'metadata.get("transcript")' in content, \
            "Should also check metadata for transcript field"

    def test_retranscribe_extracts_embed_url(self):
        """Verify retranscribe extracts embed_url from metadata."""
        collect_path = Path(__file__).parent.parent / "backend" / "routes" / "collect.py"
        content = collect_path.read_text()

        assert '"embed_url": metadata.get("embed_url")' in content, \
            "Should extract embed_url from video metadata"

    def test_retranscribe_passes_metadata(self):
        """Verify retranscribe passes full metadata to transcription."""
        collect_path = Path(__file__).parent.parent / "backend" / "routes" / "collect.py"
        content = collect_path.read_text()

        # Check that metadata is passed in the retranscribe function
        assert 'metadata=video.get("metadata")' in content, \
            "Should pass metadata to _transcribe_video_async"

    def test_retranscribe_returns_stats(self):
        """Verify retranscribe returns useful statistics."""
        collect_path = Path(__file__).parent.parent / "backend" / "routes" / "collect.py"
        content = collect_path.read_text()

...
- [ ] Frontend XSS vectors mitigated (innerHTML → textContent)
...
### Part F: Frontend XSS Mitigation

**Issue**: Frontend uses `innerHTML` to inject content, which could execute scripts if content contains malicious HTML.

**Affected Files**: `frontend/js/*.js`, `frontend/*.html`

**Current Pattern**:
```javascript
element.innerHTML = untrustedContent;  // XSS risk
```

**Mitigation Options**:

1. **Use textContent for text-only fields**:
```javascript
element.textContent = untrustedContent;  // Safe - no HTML parsing
```

2. **Sanitize HTML before injection** (if HTML needed):
```javascript
...
**Recommendation**: Since content comes from trusted research sources, use `textContent` for all text fields and only use `innerHTML` for pre-sanitized server content.
...
### Task 15.7: Frontend XSS Mitigation
**Estimate**: 1 hour

**Actions**:
1. Audit all `innerHTML` usage in `frontend/js/*.js`
2. Replace with `textContent` where only text is needed
3. Add `sanitizeHTML()` helper for cases needing HTML
4. Test that UI still renders correctly
5. Document safe HTML injection patterns
...
- `frontend/js/*.js` (XSS mitigation - innerHTML → textContent)
- `frontend/*.html` (XSS mitigation where needed)

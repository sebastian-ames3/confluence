...
**Version**: 1.1
**Date**: 2025-11-18
**Updated**: 2025-11-18 (Discord collection strategy clarified)
**Phase**: 1 - Foundation
**Duration**: 3 days
**Status**: Not Started
...
- [ ] Discord channel configuration system in place
...
class DiscordSelfCollector:
    ...
    async def collect_channel(self, channel_config):
        ...
        if not channel:
            print(f"⚠️  Channel {channel_config['name']} not found!")
            return []
        ...
        print(f"📡 Collecting from #{channel_config['name']}...")
        ...
        print(f"✅ Collected {message_count} messages from #{channel_config['name']}")
    ...
    async def process_attachments(self, attachments, collect_types):
        ...
        for attachment in attachments:
            ...
            if size_mb > file_settings["max_file_size_mb"]:
                print(f"⚠️  Skipping {attachment.filename} (too large: {size_mb:.1f}MB)")
                continue
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(railway_url, json=collected_data) as response:
                if response.status == 200:
                    print("✅ Data uploaded to Railway successfully")
                    return True
                else:
...
async def main():
    ...
    try:
        ...
        print(f"\n📊 Total messages collected: {len(collected_data)}")
        ...
        if collected_data:
            ...
            if success:
                ...
                print("\n⚠️  Collection succeeded but upload failed")
...
Sebastian's Laptop (Discord always running)
    ...
    │   └─ discord_local.py runs
    │       ├─ Reads config/discord_channels.json
    │       ├─ Connects to Discord (Sebastian's session)
    │       ├─ Collects from 5 channels
    │       ├─ Downloads PDFs/images
    │       ├─ Extracts video links
    │       └─ POSTs JSON to Railway API
    │
    ├─ 6:00 PM: Task Scheduler triggers (repeat)
    │
    └─ If laptop was off: Runs on next startup

Railway Backend
    │
    ├─ /api/ingest/discord endpoint receives data
    ├─ Validates and saves to raw_content table
    ├─ Files stored in persistent volume
    └─ Triggers Content Classifier Agent
```
